...
- 复用 `create_backend`,不新增工厂;`VisionBackend.close()` 同时关闭同步/异步 client。
...
Run: `.venv/bin/python -m pytest tests/test_composer.py tests/test_public_api.py -q 2>/dev/null || .venv/bin/python -m pytest tests/test_composer.py -q`
Expected: 全部 PASS(含原有用例)
...
- Test: `tests/test_server/test_app.py`(回归,预期无需改动)
